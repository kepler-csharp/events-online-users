  ...
  public function showEvents()
    {
        try {
            ...
            $showtimesResponse = Http::get(config('services.auth_service.url') . '/api/showtimes');
        } catch (\Exception $e) {
        ...
        if (!$eventsResponse->successful() || !$showtimesResponse->successful()) {
            return response()->json([
                'error' => 'Failed to fetch events'
            ], 500);
        }

        $events = $eventsResponse->json()['data']['items'];
        $showtimes = $showtimesResponse->json()['data']['items'];

        foreach ($events as &$event) {

            $eventShowtimes = collect($showtimes)
                ->where('eventId', $event['id']);

            $availableShowtimes = $eventShowtimes
                ->filter(function ($showtime) {
                    return $showtime['status'] == 0
                        && \Carbon\Carbon::parse($showtime['endTime'])->isFuture();
                })
                ->sortBy('startTime')
                ->values();

            $expiredShowtimes = $eventShowtimes
                ->reject(function ($showtime) {
                    return $showtime['status'] == 0
                        && \Carbon\Carbon::parse($showtime['endTime'])->isFuture();
                })
            ...
            $event['availableShowtimes'] = $availableShowtimes;
            $event['expiredShowtimes'] = $expiredShowtimes;

            $event['nextShowtime'] = $availableShowtimes->first()
                ?? $expiredShowtimes->first();
        }

        usort($events, function ($a, $b) {

            $dateA = isset($a['nextShowtime'])
                ? strtotime($a['nextShowtime']['startTime'])
                : PHP_INT_MAX;

            $dateB = isset($b['nextShowtime'])
                ? strtotime($b['nextShowtime']['startTime'])
                : PHP_INT_MAX;
        ...
        $featuredShowtime = $featuredEvent['nextShowtime'] ?? null;
